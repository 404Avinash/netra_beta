...
3. **Highlight Key Features** (5 min)
...
## 🛠️ Technology Stack## 🏗️ System Architecture
...
## 🔐 Security & Classification- **Manipur**: 10 locations (Imphal, Moreh Border, Thoubal)  - Metal detectors
...
## 🏗️ System Architecture  - Multispectral imaging
...
│  Sensor Layer   │ ───▶ │   AI Processing  │ ───▶ │  Web Dashboard  │- Mounted on mobile command vehicle

- Indian Army - Corps of Engineers (Military Engineering Services)

- North-East India border security initiatives│  (7 Sensors)    │      │  (Bayesian Fusion)│      │  (Streamlit)    │- Aggregates data from edge devices
...
**🛡️ N.E.T.R.A. - Protecting Lives Through Intelligent Threat Detection**
...
</div>```- Historical threat data analysis
...
1. **Sensor Acquisition** → 7 sensors collect data (fume, metal, GPR, CV, disturbance, thermal)

2. **Data Validation** → Real-time filtering and normalization (0-100% range)## 🔍 Detection Workflow

3. **AI Analysis** → Bayesian fusion with correlation boosting

4. **Threat Scoring** → 0-100% probability + CRITICAL/HIGH/MODERATE/LOW classification### Phase 1: Rover Ground Scan

5. **Storage** → CSV logs for historical analysis```

6. **Bridge Layer** → Data connector for StreamlitChemical Fume Analysis → Metal Detection → GPR Subsurface Scan → Ground-Level Vision

7. **Visualization** → Interactive dashboard with maps, charts, controls                                    ↓
...
## 🔧 Technical Stack### Phase 2: Drone Aerial Verification

```

### Core TechnologiesHigh-Res Visual Scan → Thermal Analysis → Context Classification
...
### AI/ML ComponentsProbabilistic Fusion (Bayesian) → Threat Confirmation (>75%) → Action Decision
...
### Data FormatEvacuation Protocol → Microwave Deployment (100W/cm²) → Outcome Logging
...
## 📖 Usage Guide| Phase | ML Model | Architecture | Output |
...
Fume Detection:    ████████░░ 85%- **30,000** aerial images (IED components, footprints)

Metal Detection:   ███████░░░ 80%- **25,000** annotated terrain images

GPR Analysis:      ██████████ 75%- **8,000** thermal images
...
Thermal:           ██████░░░░ 65%## 💻 Tech Stack

```

### Edge Computing

**Click "ANALYZE THREAT NOW"** → Get instant threat assessment:- **NVIDIA Jetson Xavier NX** (Rover)
...
- **Color-coded markers**:- **XGBoost** - Gradient boosting
  ...
  - 🔴 RED = CRITICAL threat- **pgmpy** - Bayesian networks
...
- Displays sorted threat list (highest → lowest)### Frontend
...
**Scenario 2: Urban Area**2. **Install Python dependencies**
...
**Scenario 3: Military Base**3. **Install frontend dependencies**
...
## 📁 Project Structurecd fusion_engine
...
├── netra_streamlit_app.py          # Main Streamlit dashboard### Running with Docker

├── requirements.txt                 # Python dependencies

│```bash

├── netra_threat_log.csv            # 500 threat analyses (30 days)docker-compose up -d

├── locations_northeast_india.csv   # 50 strategic locations```

├── sensor_readings_live.csv        # 100 sensor readings

│This will start:

├── generate_datasets.py            # Dataset generation script- Fusion Engine API (port 8000)

├── test_datasets.py                # Dataset compatibility tests- Dashboard (port 3000)

│- PostgreSQL database (port 5432)

├── README.md                       # This file- Redis (port 6379)

├── DATA_FLOW_COMPLETE_GUIDE.md    # Technical documentation

├── DATA_FLOW_VISUAL_DIAGRAMS.txt  # Architecture diagrams---

│

├── .gitignore                      # Git exclusions## 👥 Team
...
1. **Chemical + Metal** (fume > 70% AND metal > 70%) → +10% boost
2. **Triple Detection** (fume + metal + GPR all > 60%) → +15% boost
3. **Visual Confirmation** (ground_cv + drone_cv both > 70%) → +12% boost
4. **Thermal + Disturbance** (both > 60%) → +8% boost
5. **Full Spectrum** (5+ sensors > 50%) → +20% boost

### Threat Classification

```python
if probability >= 75:  → CRITICAL
elif probability >= 55: → HIGH
elif probability >= 35: → MODERATE
else:                   → LOW
```
...
- **GitHub**: [@404Avinash](https://github.com/404Avinash)
- **Repository**: [netra_beta](https://github.com/404Avinash/netra_beta)
- **Issues**: [Report a bug or request a feature](https://github.com/404Avinash/netra_beta/issues)
...
- **[Data Flow Guide](DATA_FLOW_COMPLETE_GUIDE.md)**: Complete technical documentation
- **[Visual Diagrams](DATA_FLOW_VISUAL_DIAGRAMS.txt)**: System architecture diagrams
...
✅ **Full Stack**: Sensor → AI → Dashboard  
...
**⚡ Built with Streamlit • Powered by Python • Designed for Security**
...
### 🛡️ N.E.T.R.A. - Protecting Lives Through Intelligence

*Next-Gen Eye for Threat Recognition and Analysis*

[View Demo](https://share.streamlit.io) • [Report Issue](https://github.com/404Avinash/netra_beta/issues) • [Documentation](DATA_FLOW_COMPLETE_GUIDE.md)
